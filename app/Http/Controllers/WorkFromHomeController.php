...
class WorkFromHomeController extends Controller
{
    public function index(){
        $data['page_title'] = 'WORK FROM HOME';

        return view('absensi.work-from-home.index', $data);
    }
    public function getAbsensi(Request $request){
        try {
          ...
          // Get data jenis absensi berdasarkan type = Work From Home, Timestamp = Hari ini.  
          $getDataWFH = Absensi::where('user_id',$request->user_id)->where('type_absensi', 'Work From Home')->where('timestamp', 'like', '%' . date('Y-m-d') . '%')->select('jenis_absensi')->get();
          
          // Membuat array untuk kebutuhan pengecheckan berdasarkan jenis absensi
          $tempJenisAbsensi = [];
          foreach ($getDataWFH as $key => $value) {
          ...
          if (in_array('masuk',$tempJenisAbsensi) && !in_array('pulang',$tempJenisAbsensi)) {
              $getDataWFHByJenis = Absensi::where('user_id',$request->user_id)->where('jenis_absensi', 'masuk')->where('type_absensi', 'Work From Home')->where('timestamp', 'like', '%' . date('Y-m-d') . '%')->orderBy('timestamp','desc')->first();
              
              $timestampMasuk = '-';
          }else if(in_array('pulang',$tempJenisAbsensi)){
              $getDataWFHByJenis = Absensi::where('user_id',$request->user_id)->where('jenis_absensi', 'pulang')->where('type_absensi', 'Work From Home')->where('timestamp', 'like', '%' . date('Y-m-d') . '%')->orderBy('timestamp','desc')->first();
              $getTimestampMasuk = Absensi::where('user_id',$request->user_id)->where('type_absensi', 'Work From Home')->where('jenis_absensi', 'masuk')->where('timestamp', 'like', '%' . date('Y-m-d') . '%')->orderBy('timestamp','desc')->select('timestamp')->first();
          ...
          // ====================
          // Get Data (Lama Kerja) 
          $jam_masuk 		  = date_create($getDataWFHByJenis->timestamp);
